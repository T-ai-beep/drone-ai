...
def build_styles():
    ...
    styles = {
        'title': ParagraphStyle('BeaconTitle', fontSize=28, fontName='Helvetica-Bold',
                                textColor=colors.HexColor('#0A0D14'), spaceAfter=4, leading=32),
        'subtitle': ParagraphStyle('BeaconSubtitle', fontSize=13, fontName='Helvetica',
                                   textColor=colors.HexColor('#374151'), spaceAfter=2, leading=16),
        'section': ParagraphStyle('BeaconSection', fontSize=11, fontName='Helvetica-Bold',
                                  textColor=colors.HexColor('#0A0D14'), spaceBefore=16, spaceAfter=6, leading=14),
        'body': ParagraphStyle('BeaconBody', fontSize=9, fontName='Helvetica',
                               textColor=colors.HexColor('#374151'), spaceAfter=3, leading=13),
        'mono': ParagraphStyle('BeaconMono', fontSize=8, fontName='Courier',
                               textColor=colors.HexColor('#374151'), spaceAfter=2, leading=11),
        'alert': ParagraphStyle('BeaconAlert', fontSize=9, fontName='Helvetica-Bold',
                                textColor=colors.HexColor('#EF4444'), spaceAfter=3, leading=13),
        'good': ParagraphStyle('BeaconGood', fontSize=9, fontName='Helvetica-Bold',
                               textColor=colors.HexColor('#22C55E'), spaceAfter=3, leading=13),
    }
    return styles
...
def generate_report(data: MissionData) -> str:
    ...
    if data.survivor_candidates:
        ...
        for i, c in enumerate(data.survivor_candidates, 1):
            cand_data.append([str(i), f"{c.get('lat', 'N/A')}", f"{c.get('lon', 'N/A')}",
                              f"{c.get('bearing_deg', 'N/A')}°", f"{c.get('confidence', 0)*100:.0f}%",
    ...
    if data.decisions:
        ...
        for d in data.decisions:
            dec_data.append([d.get('time', ''), d.get('recommendation', ''), d.get('outcome', '')])
    ...
    if data.models_active:
        ...
        content.append(Paragraph(" · ".join(data.models_active), styles['mono']))
    content.append(Spacer(1, 8))
